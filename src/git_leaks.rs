...
/// [`REPO`] 중 **설정 파일의 자리를 돌리는 것** 셋 — 이름은 위에 든 것과 같은 것을 가리킨다.
///
...
///
/// **되돌리는 것은 이 셋뿐이고, 그 자리도 하나뿐이다.** `GIT_DIR` 무리까지 물려주면 훅이 준
/// 환경이 **어느 저장소를 여는가**를 바꾸고, 그것을 막는 것이 걷기가 선 까닭이다
/// (`git::run_reading_user_config` 가 그 경계를 지킨다).
pub const CONFIG_FILES: &[&str] = &["GIT_CONFIG_GLOBAL", "GIT_CONFIG_SYSTEM", "GIT_CONFIG"];
