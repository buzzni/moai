...
/// [`REPO`] 중 **병합에서도 읽히는 설정 자리**를 돌리는 것 둘 — 이름은 위에 든 것과 같은 것을
/// 가리킨다.
///
/// 늘 걷지만, **딱 한 물음만 이것을 도로 물려받아 묻는다**(moai-b5np): "git 이 병합에서 이
/// 드라이버를 찾을 수 있는가"(`merge_driver::planted_anywhere`). 그 물음의 답은 **git 이 실제로
/// 읽을 파일**에서 나와야 참이고, 걷은 채로 물으면 이 변수로 전역 설정을 딴 파일에 둔 사람에게
/// "안 심었다 — 병합이 기본 머지로 내려앉는다" 라는 거짓말을 한다(실제로 그 줄은 돈다).
///
/// **`GIT_CONFIG` 는 여기 안 든다**(리뷰). 위의 둘은 **자리 하나를 옮기고** 나머지 자리는 그대로
/// 두는데, `GIT_CONFIG` 는 `git config` 가 보는 **전부를 그 파일 하나로 갈아 치운다** — 그리고
/// 병합은 그 변수를 아예 안 읽는다(git 2.43 에서 쟀다: `--local` 에 심은 값이 `git config --get`
/// 에서 사라지지만 `git var` 는 그대로 제 값을 낸다). 셋을 함께 물려주면 이 물음이 **git 이 병합
/// 때 안 볼 파일** 하나만 보고 답한다: 그 파일에 키가 없으면 실제로 도는 드라이버를 "안 심었다"
/// 고 조르고(moai-b5np 가 없애려던 바로 그 헛 알림), 있으면 없는 드라이버를 조용히 덮는다.
///
/// **되돌리는 것은 이 둘뿐이고, 그 자리도 하나뿐이다.** `GIT_DIR` 무리까지 물려주면 훅이 준
/// 환경이 **어느 저장소를 여는가**를 바꾸고, 그것을 막는 것이 걷기가 선 까닭이다
/// (`git::run_reading_user_config` 가 그 경계를 지킨다).
pub const CONFIG_FILES: &[&str] = &["GIT_CONFIG_GLOBAL", "GIT_CONFIG_SYSTEM"];
