...
pub fn run(ctx: &Ctx, args: TuiArgs) -> R<Vec<String>> {
    ...
    app.adopt_look(&reg.look, look_problems(&reg, app.site.lang));
...
/// 보기를 읽다 만난 까닭과 **옛 `[read]` 에서 건너뛴 줄**을 한 알림으로(moai-rtji).
///
/// 둘은 한 알림에 서지만 오는 꼴이 다르다 — 보기의 까닭은 글로, 옛 `[read]` 의 것은 자료로 온다
/// ([`crate::user_config::Registry::read_problems`]). 설정을 읽는 자리는 화면 말을 안 묻으므로
/// 펴는 것이 여기다. **자리를 머리에 붙인다** — 한때 설정 쪽이 글에 붙이던 그 꼴(`{설정}: …`)이라,
/// 안 붙이면 "손으로 고친다" 가 어느 파일인지 모르는 말이 된다.
///
/// **말은 화면이 이미 든 것이다**(`App::site.lang`) — 여기서 `ctx.lang()` 을 다시 물으면 밖에서
/// 띄운 길(`outside`)이 같은 설정을 한 번 더 판다(`super::lang_of` 가 적어 둔 그 까닭이다).
fn look_problems(reg: &crate::user_config::Registry, lang: crate::i18n::Lang) -> Vec<String> {
    let skipped = reg.read_problems.iter().map(|why| {
        let said = crate::view::skipped(lang, why);
        match reg.path.as_deref() {
            Some(at) => format!("{}: {said}", at.display()),
            None => said,
        }
    });
    reg.look_problems.iter().cloned().chain(skipped).collect()
}
