...
//! 없다. 설정 디렉터리가 링크인 것은 커널이 알아서 지나가므로 여기서 풀 것이 없다.
...
pub fn path_for(config: &Path, root: &Path) -> PathBuf {
    dir_of(config).join("read").join(format!("{:016x}.toml", fnv1a(root.as_os_str().as_encoded_bytes())))
}
...
pub fn read(config: &Path, root: &Path, legacy: &BTreeMap<String, String>) -> (BTreeMap<String, String>, Vec<String>) {
    ...
    for (id, when) in legacy {
        marks.entry(id.clone()).or_insert_with(|| when.clone());
    }
    (marks, problems)
}
...
impl Sheet {
    fn parse(src: &str) -> Result<Sheet, String> {
        let doc = src.parse::<toml_edit::DocumentMut>().map_err(|e| e.to_string())?;
        Ok(Sheet { doc, dirty: false })
    }

    /// 이 파일이 그 뿌리의 것인가. `path` 가 없으면(처음 짓는 파일) 참이다 — 그때 [`Sheet::claim`] 이 적는다.
    fn owns(&self, root: &Path) -> bool {
        match self.doc.get(PATH).and_then(Item::as_str) {
            None => true,
            Some(at) => Path::new(at) == root,
        }
    }
    ...
    fn claim(&mut self, root: &Path) {
        ...
        self.doc.insert(PATH, Item::Value(root.display().to_string().into()));
        self.dirty = true;
    }
    ...
    pub fn mark(&mut self, marks: &BTreeMap<String, String>) -> R<Vec<String>> {
        ...
        if let Some(item) = self.doc.get(READ) {
            let Some(t) = item.as_table_like() else {
                return Err(Fail::coded(
        ...
        for (id, when) in marks {
            if t.get(id).and_then(Item::as_str) != Some(when.as_str()) {
                t.insert(id, Item::Value(when.as_str().into()));
    ...
    pub fn prune(&mut self, known: &BTreeSet<&str>) -> usize {
        let Some(t) = self.doc.get_mut(READ).and_then(Item::as_table_like_mut) else {
            return 0;
        };
        let gone: Vec<String> = t.iter().map(|(id, _)| id.to_string()).filter(|id| !known.contains(id.as_str())).collect();
        for id in &gone {
            t.remove(id);
        }
...
/// 파일이 든 디렉터리. 디렉터리 조각이 없는 상대 철자면 `.` 이다.
fn dir_of(path: &Path) -> &Path {
    path.parent().filter(|d| !d.as_os_str().is_empty()).unwrap_or(Path::new("."))
}

/// FNV-1a 64비트. **직접 적는다** — `DefaultHasher` 는 표준이 값을 바꿀 수 있다고 적어 둔 것이라, 러스트를
/// 올리는 것만으로 모든 사람의 읽음 파일 이름이 바뀌어 읽음이 통째로 사라진다. 여기 적힌 셈은 안 바뀐다.
...
mod tests {
    ...
    /// **셈은 안 바뀐다** — 이름이 바뀌면 모든 사람의 읽음이 한 번에 사라진다. 값을 글자로 못박는다.
    #[test]
    fn the_hash_is_pinned_so_the_names_never_move() {
        assert_eq!(fnv1a(b""), 0xcbf2_9ce4_8422_2325);
        assert_eq!(fnv1a(b"a"), 0xaf63_dc4c_8601_ec8c);
        assert_eq!(fnv1a(b"/home/coder/workspace/moa-issue"), fnv1a(b"/home/coder/workspace/moa-issue"));
        assert_ne!(fnv1a(b"/a/api"), fnv1a(b"/b/api"));
    }
}
