...
- 그런 일이 있으면 **새 idea 보다 먼저** 놀고 있는 세션 하나에 이어 하기를 맡긴다. 3 의
  글 대신 아래를 싣고, 그 뒤에 3 의 글의 5~11 을 **통째로** 잇는다
...
fn brief() -> String {
    ...
    4. 노트에 없는 설계 결정은 추측하지 말고 AskUserQuestion 으로 묻는다 —
       사람이 일꾼 창을 보고 있다
    4-1. **트래커는 언제나 루트의 것을 고친다.** 쓰는 명령(`add`·`idea add`·
       `note`·`mv`)을 워크트리 안에서 맨 `moai` 로 부르면 그 워크트리의 `.moai` 가 바뀌어, 병합할 때 스냅샷이 충돌한다
       (합쳐도 남의 줄을 덮는다). 워크트리에서는 `moai -C <루트> <명령>` 으로 부르고,
       **리뷰 서브에이전트에게도** 같은 말을 준다 — 넘긴 것을 담다가 그 줄을 워크트리에
       적은 적이 있다. 이미 적었으면 `git checkout -- .moai` 로 되돌리고 루트에서 다시 담는다
