...
(`--parent <id>`)으로 만든다. 그 일이 내건 것이 이것 없이 안 이뤄지면 지금 못
...
const RECALL: &str = "moai idea promote <idea id> -e <에픽> --from -";
...
{rubric}
    ...
    7-1. 병합 전에 이 창이 에픽 도중 담은 idea 와 리뷰가 넘긴 것을 되짚는다 — **에픽이 내건
       ...
       멤버로 펼친다 — 계획에는 `- 이슈` 줄만 적고, idea 는 저절로 닫히며 출처가 남는다. 4-1 대로
       `moai -C <루트>` 로 친다
         {recall}
