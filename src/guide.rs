...
**사람 화면의 `옆 워크트리 문제 N건` 은 이것과 다른 수다.** 그쪽은 스냅샷을 못 읽은 워크트리를
...
말이지 못 셌다는 말이 아니다 — 못 셌는지는 `unreadable_worktrees` 하나가 답한다.
...
유일한 리뷰다. 비싼 눈으로 전체를 본다(브리프 5·7 이 일꾼에게 싣는다). **망설여지면 한 단계 올린다.** 감독은 코드를
...
{model}
    ...
    5. **멤버마다 리뷰하지 않는다.** 멤버 하나가 끝나면 시험을 돌리고 커밋해 다음 멤버로
       간다 — 리뷰는 멤버가 다 끝난 뒤 7 에서 에픽 전체를 한 번 본다. 멤버마다 보던 판에서
       멤버 리뷰는 판당 1~3건, 에픽 끝은 9~15건이었고 실제 버그도 거의 다 에픽 끝이 찾았다 —
       두 번 보는 값이 곱으로 들었다. 멤버 1 의 버그 위에 멤버 2 가 쌓이는 값은 그 한 번에서
       같이 고친다.
       {levels} 은 머리의 모델을 고른 잣대이고, 7 의 등급을 고를 때도 이것으로 멤버를 잰다.
{rubric}
       7 의 리뷰가 낸 것의 반영은 별도 fix: 커밋, 넘긴 것은 이슈 번호와 함께 노트.
    ...
    7. 병합 전에 에픽 전체를 `/code-review <xhigh|max> --fix` 로 본다 — 멤버를 따로 안 봤으니
       이 한 번이 유일한 리뷰다. 멤버가 서로 거의 안 닿고 쓰기·저장·훅을 안 건드렸으면 xhigh,
       표면을 가로지르거나 설계 결정이 여럿이거나 **쓰기 경로·저장 형식·훅을 건드린 멤버가
       하나라도 있으면** max. 망설여지면 max 다. 이 리뷰는 머리의 모델과 상관없이 `{top}` 로 본다 — 창이
       `{top}` 가 아니면 부르기 전에 창을 보는 사람에게 `/model {top}` 를 청한다(리뷰 에이전트는
       창의 모델을 물려받는다). 고른 등급과 까닭은 관점(`-b`)에 한 줄로 적는다. 가지가 <본 가지> 를 떠난
