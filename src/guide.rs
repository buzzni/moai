...
**5. {five}.** 딸린 워크트리의 스냅샷을 고치는 `moai`
(`add`·`mv`·`edit`·`rm`·`note`·`defer`·`link`·`idea promote` 와 그 네임스페이스)를 막는다 —
그 안에서 친 줄도, 밖에서 `-C <워크트리>` 로 겨눈 줄도 같다. 그 `.moai` 를 고치면 병합에서
스냅샷이 충돌해, 푸는 길이 도구 밖에만 남는다. 거절문이 **그 워크트리의 루트로 겨눈 같은
줄**을 낸다 — 사람을 부르지 않는다. 읽기(`status`·`ready`·`show`)는 어디서든 자유롭다.
탐색기(`tui`)도 안 막는다 — 다만 `SPC n` 은 선 자리에 쓰니, 워크트리에서 담은 생각은
거기 남는다."#
    )
}
