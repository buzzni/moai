...
mod tests {
    ...
    fn the_milestone_first_rule_stands_in_all_three_teachings() {
        ...
        for (piece, why) in [
            ("마일스톤이 도는 동안 그 안이 먼저다", "AGENTS 블록에 규칙이 없다"),
            ("시작한\n칸의 멤버가 하나라도 있으면 도는 것이다", "무엇으로 시작을 재는지 안 적었다"),
            ("**`p0` 은 마일스톤과 무관하게 밖에 있어도 집는다**", "핫픽스 자리를 안 적었다"),
            ("**막지 않는다.** 밖의 일을 집는 `moai mv` 는 그대로 지나간다", "막지 않는다는 것을 안 적었다"),
        ] {
            assert!(agents.contains(piece), "{why} — {piece}");
        }
        for (piece, why) in [
            ("마일스톤이 돌면 그 안이 먼저다", "감독이 고르는 자리에 규칙이 없다"),
            ("도구는 이것을 막지 않는다", "감독이 도구가 막아 줄 것으로 읽는다"),
        ] {
        ...
        for (piece, why) in [
            ...
            ("`#bug` 태그가 붙는가", "버그 수준을 있는 낱말로 안 잰다"),
            ("`Regression-of:` 줄을 적을", "되돌림을 안 센다"),
        ] {
            assert!(brief.contains(piece), "{why} — {piece}");
        }
    }
