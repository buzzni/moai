...
/// 도움말을 **플래그 자리에서** 부르는가 — [`positionals`] 와 같은 자로 훑는다.
///
/// [`asks_help`] 는 낱말이 어디 있든 센다. 비추거나 세는 자리에서는 그것으로 족했지만,
/// **막는 자리에서는 그 너그러움이 구멍이다** — `moai note <id> -- --help` 는 `--help` 를 글로
/// 적는 쓰기인데 그쪽은 도움말로 읽어 규칙 5 를 통째로 지나갔고, `moai mv <id> done -m -h` 도
/// 같다(`-m` 의 값이다). 값과 `--` 뒤를 가르는 자는 이미 [`positionals`] 가 쓴다.
fn asks_help_flag(args: &[String]) -> bool {
    let mut it = args.iter();
    while let Some(a) = it.next() {
        if a == "--" {
            return false;
...
pub struct Away {
    ...
    /// 이 세션이 마지막으로 집은 줄([`Picks::mine`]) — `unsure` 가 그 조상을 가리켜도 빼지 않는다(리뷰
...
/// 집은** 세션이 이 세션이면 `mine`, 다른 세션이면 `theirs` 다. 적는 것과 읽는 것은 `cmd/hook.rs`
/// 가 하고, 여기는 그 답만 받는다.
///
...
impl Picks {
    ...
    /// 집기 한 번을 적는 줄 — `때\tid\t초`. `때` 는 차례를 가르는 벽시계 나노초, `초` 는 줄의 칸 시각과
    /// 견줄 `model::now` 의 초다(못 읽었으면 뺀다). 읽는 쪽은 [`Picks::fold`] 하나다 — 꼴이 두 자리에
    /// 서면 한쪽만 고쳐지는 날 기록이 통째로 안 읽힌다.
    pub fn line(at: u128, stamp: Option<i64>, id: &str) -> String {
        match stamp {
            Some(secs) => format!("{at}\t{id}\t{secs}\n"),
            None => format!("{at}\t{id}\n"),
        }
    }
    ...
    pub fn fold<S: AsRef<str>, T: AsRef<str>>(
        ...
        // id → (이 세션의 마지막 때, 다른 세션들의 마지막 때). `None` 은 어느 때보다 이르다.
        let mut last: std::collections::BTreeMap<String, (Option<u128>, Option<u128>)> = Default::default();
        for (sid, text) in files {
            ...
            for line in text.as_ref().split_inclusive('\n') {
                ...
                let (m, t) = last.entry(id.to_string()).or_default();
                let side = if mine { m } else { t };
                *side = (*side).max(Some(at));
            }
        }
...
/// 옮긴) 줄(`elsewhere`, `worktree::held_elsewhere`)과 **다른 세션이 마지막으로 집은** 줄(`picks`)이다
...
/// 다른 세션이 마지막으로 집었으면 이름과 상관없이 모르는 것이고, 이 세션이 마지막으로 집었으면
/// 옆 스냅샷에 있어도 제 것이다.
pub fn unsure(
    issues: &[Issue],
    cfg: &Config,
...
fn writes_tracker(seg: &[String]) -> bool {
    ...
    let Some(args) = moai_args(seg) else { return false };
    // **도움말과 연습(`--dry-run`)은 아무것도 안 넣는다** — [`writes_text`] 와 같은 자다. `--dry-run`
    // 을 안 빼던 판은 `moai add --from - --dry-run` 을 막았다. 그것은 에픽을 세우기 **전에** 사람에게
    // 안을 보이는 걸음(AGENTS.md 갈림길 3)이라, 막으면 남는 길이 진짜로 세워 보는 것뿐이다.
    if asks_help_flag(args) || args.iter().any(|a| a == "--dry-run") {
        return false;
    }
    let verbs = positionals(args);
...
///
/// **막되 사람을 안 부른다** — 거절문이 **그 워크트리의 루트를 `-C` 로 겨눈 같은 줄**을 낸다.
/// 어디에 서서 쳤는지는 안 본다: 워크트리 안에서 친 줄도, 루트에서 `-C <옆 워크트리>` 로 겨눈
/// 줄도, 남의 프로젝트의 워크트리를 가리킨 줄도 다 그 줄이 가리킨 트래커의 main 으로 간다.
///
/// `root(k)` 는 그 토막이 겨눈 트래커가 딸린 워크트리면 **그것을 쓸 주 체크아웃의 자리**를 낸다 —
/// 디스크는 부르는 쪽이 본다. 겨눌 곳을 여기서 받는 것은 **거절문이 제 손으로 `-C` 를 넣기**
/// 위해서다. 맨 줄을 내고 [`toward`] 가 뒤에서 겨누게 두던 판은 그것이 안 겨누는 자리마다
/// 막힌 줄을 그대로 도로 냈다 — `--dir=<자리>`·`-C<자리>`(뗄 줄 몰라 `toward` 가 건너뛴다),
/// `./target/release/moai`(맨 `moai` 가 아니라 건너뛴다), 루트가 모르는 id([`unsynced`]).
/// 셋 다 "이대로 치면 지나간다" 아래 막힌 줄이 그대로 서는 고리였다.
///
/// **`None` 이면 안 막는다.** 주 체크아웃이 없거나(맨 저장소에 딸린 워크트리·서브모듈 —
/// [`crate::worktree::is_linked`] 는 `.git` 이 파일이기만 하면 참이다) 거기에 트래커가 없으면
/// 겨눌 곳이 없다. 그때 막으면 거절문이 낼 수 있는 줄은 막힌 줄뿐이라 나갈 길이 도구 밖에만
/// 남는다 — 그건 게이트다(`Decision::Deny` 의 계약).
pub fn guard_tracker(cmd: &str, root: &dyn Fn(usize) -> Option<PathBuf>) -> Decision {
    // 훅은 도구 호출마다 돈다 — `moai` 라는 글자도 없으면 명령줄을 다시 가르지 않는다([`picked_in`]
    // 과 같은 자다). [`writes_tracker`] 는 명령 자리의 basename 이 `moai` 일 때만 참이다.
    if !cmd.contains("moai") {
        return Decision::Pass;
    }
    // **막는 토막을 모두 낸다.** 첫 것에서 멈추던 판은 `moai note <id> '…' && moai mv <id> done`
    // 의 뒤 절반을 지웠다 — 시킨 대로 친 쪽은 노트만 남기고 일이 닫힌 줄로 알았다.
    let lines: Vec<String> = segments(cmd)
        .into_iter()
        .enumerate()
        .filter(|(_, seg)| writes_tracker(seg))
        .filter_map(|(k, seg)| Some(as_typed(&seg, &root(k)?)))
        .collect();
    if lines.is_empty() {
        return Decision::Pass;
    }
    // **"지나간다" 고 하지 않는다.** 이 규칙은 규칙 1~3 보다 **먼저** 서므로, 여기를 지난 줄이
    // 초점 밖의 생성이면 곧바로 규칙 1 이 막는다 — `moai add '딴 일'` 이 그 자리다. "이대로 치면
    // 지나간다" 는 그때 거짓이 되고, 막힌 쪽은 훅이 제 말을 뒤집는 것으로 읽는다.
    refuse(5, format!(
        "여기는 딸린 워크트리다 — 이 줄은 워크트리의 `.moai` 를 고쳐 병합에서 스냅샷이 충돌한다.\n\
         트래커는 루트에 쓴다. 이 줄을 루트로 겨눠 다시 친다 — 리다이렉션(`< 파일`·heredoc)과 파이프는 그대로 둔다:\n\n  {}",
        lines.join(" && "),
    ))
}

/// 토막을 **루트로 겨눠 다시 칠 수 있는 한 줄로** — 적힌 `-C`·`--dir` 은 떼고 그 자리에 `root` 를
/// 넣는다. **clap 이 받는 네 꼴을 다 뗀다**(`-C 값`·`-C값`·`-C=값`·`--dir=값`): 자리를 읽어 막은
/// 것이 그 넷을 다 보는 [`flag_values`] 라, 하나라도 남기면 거절문이 막힌 줄을 그대로 도로 낸다.
///
/// **앞에 선 환경변수 대입은 남긴다**(`MOAI_ACTOR='…' moai …`). [`command_of`] 가 그것을 지나치는
/// 것은 명령 자리를 찾기 위해서지 버리기 위해서가 아니다 — 떼면 `git config` 가 없는 기계에서
/// 시킨 대로 친 줄이 "누군지 모른다" 로 멈춘다(CLAUDE.md "누군지 모르면 쓰기는 멈추고").
fn as_typed(seg: &[String], root: &Path) -> String {
    let rest = command_of(seg);
    let mut out: Vec<String> = seg[..seg.len() - rest.len()]
        .iter()
        .filter_map(|w| w.split_once('=').filter(|_| !w.starts_with(['-', '='])))
        // **이름은 감싸지 않는다** — 통째로 감싸면 셸이 대입이 아니라 그 이름의 **명령**으로 읽는다.
        .map(|(name, value)| format!("{name}={}", typed_word(value)))
        .collect();
    let mut words = rest.iter();
    if let Some(head) = words.next() {
        out.push(typed_word(head));
        out.push("-C".to_string());
        out.push(crate::text::shell_word(&root.display().to_string()));
    }
    let mut skip = false;
    let mut flags = true;
    for w in words {
        ...
        // `--` 뒤는 자리 인자다 — [`flag_values`]·[`positionals`] 와 같은 자다. 안 가르면
        // `moai note <id> -- --dir=x` 의 글이 플래그로 읽혀 통째로 사라진다.
        if flags && (w == "-C" || w == "--dir") {
            skip = true;
            continue;
        }
        if flags && (w.starts_with("-C") || w.starts_with("--dir=")) {
            continue;
        }
        flags &= w != "--";
        out.push(typed_word(w));
    }
    out.join(" ")
}

/// 낱말 하나를 **옮겨 친 셸이 처음처럼 읽게** 감싼다.
///
/// [`crate::text::shell_word`] 를 안 쓴다: 그쪽은 경로를 감싸는 자라 `-` 로 시작하는 낱말에 `./` 를
/// 붙인다 — 명령줄을 다시 짜는 여기서는 그 낱말이 플래그다.
///
/// **풀 자리(`$HOME`·`$(…)`·백틱)는 큰따옴표다.** 렉서가 그것을 글자째 남기므로([`unknowable`]),
/// 작은따옴표로 싸면 시킨 대로 친 줄이 그 글자를 **글로** 쓴다 — `moai note <id> "$(cat 리뷰)"` 가
/// 리뷰 원문 대신 `$(cat 리뷰)` 아홉 글자를 저널에 적고 0 으로 끝나던 자리다. [`echo_dir`] 이
/// `-C` 값에 이미 같은 자를 쓴다(리뷰 moai-ju21.70g). 따옴표 안에서 뜻을 지는 `\`·`"` 만 막는다.
fn typed_word(w: &str) -> String {
    if w.contains(['$', '`']) {
        return format!("\"{}\"", w.replace('\\', "\\\\").replace('"', "\\\""));
    }
    // **맨 앞의 `~` 도 그대로 둔다** — [`echo_dir`] 의 갈래에 `~` 가 든 것과 같은 까닭이다. 렉서가
    // 그것을 안 풀어 남기므로([`unknowable`]), 감싸면 `moai edit <id> -b '~/notes.md'` 가 되어 옮겨 친
    // 셸이 글자 그대로의 `~` 디렉터리를 찾는다. 큰따옴표로는 안 풀리니 `$` 와 같은 자를 못 쓴다.
...
fn create_in<'a>(issues: &'a [Issue], focus: &[&'a Issue], cmd: &str, only: &dyn Fn(usize) -> bool) -> Decision {
    ...
    let makes = segments(cmd).into_iter().enumerate().filter(|(k, _)| only(*k)).map(|(_, seg)| seg).find(|seg| {
        ...
        creates(seg)
            ...
            && !asks_help(seg)
            // 단위 안에 세우는 것은 지나간다.
            && !flag_values(seg, &["-e", "--epic", "--parent", "--milestone"]).iter().any(|v| unit.contains(v.as_str()))
    });
    ...
    let aim = epics.iter().chain(&loose).copied().collect::<Vec<_>>().join("·");
...
fn shell_writes(cmd: &str, cfg: &Config, only: &dyn Fn(usize) -> bool) -> Vec<String> {
    ...
    for seg in parse(cmd) {
        ...
        for scope in [&mut strict, &mut lone, &mut sure, &mut sure_e, &mut negated_at] {
        ...
        if j.op == Op::Or && j.depth < seg.level && !picked_before {
            orelse = Some(orelse.map_or(j.depth + 1, |c| c.min(j.depth + 1)));
        }
        ...
        let bail = or && !seg.sub && matches!(command_of(&seg.words).first().map(String::as_str), Some("exit"));
        if bail {
            j.op = Op::And;
        }
...
mod tests {
    ...
    /// **규칙 5 — 트래커는 루트에서 쓴다**(moai-hwrm). 쓰는 동사만 막고, 거절문이 댄 줄은
    /// 그 워크트리의 루트를 `-C` 로 겨눈 **같은 줄**이다.
    ///
    /// 여기가 단위 시험으로 서는 까닭은, 이 규칙의 유일한 시험이 진짜 git 워크트리를 띄우는
    /// 끝-끝 시험이던 동안 **손으로 쳐 본 꼴만 덮였기** 때문이다(리뷰 moai-71ht.yid) — `-C=`·`-C값`·
    /// `--dir=` 셋이 거절문에 그대로 남아 막힌 줄을 도로 내는 고리였고, 아무것도 붉어지지 않았다.
    #[test]
    fn a_tracker_write_in_a_linked_worktree_is_aimed_at_the_root() {
        let root = Path::new("/repo");
        let at = |_: usize| Some(root.to_path_buf());
        let line = |cmd: &str| {
            let why = denied(&guard_tracker(cmd, &at)).to_string();
            assert!(why.starts_with(&crate::guide::rule_head(5)), "규칙 5 가 안 섰다 — {cmd}\n{why}");
            why.lines().last().unwrap().trim().to_string()
        };
        // **`-C` 는 clap 이 받는 네 꼴을 다 떼고 제자리에 루트를 넣는다.** 하나라도 남기면
        // 거절문이 막힌 줄을 그대로 도로 낸다 — 시킨 대로 친 쪽이 같은 자리를 맴돈다.
        for cmd in [
            "moai note t-1 '찾은 것'",
            "moai -C /w note t-1 '찾은 것'",
            "moai --dir /w note t-1 '찾은 것'",
            "moai --dir=/w note t-1 '찾은 것'",
            "moai -C=/w note t-1 '찾은 것'",
            "moai -C/w note t-1 '찾은 것'",
        ] {
            assert_eq!(line(cmd), "moai -C /repo note t-1 '찾은 것'", "{cmd}");
        }
        // 경로로 부른 바이너리도 겨눈다 — 이 저장소의 규약이 `./target/release/moai` 다.
        assert_eq!(line("./target/release/moai rm t-1"), "./target/release/moai -C /repo rm t-1");
        // 풀 자리는 큰따옴표라 옮겨 친 셸이 처음처럼 푼다. 작은따옴표로 싸면 리뷰 원문 대신
        // `$(cat 리뷰)` 라는 **글자**가 저널에 남는다.
        assert_eq!(line("moai note t-1 \"$(cat 리뷰)\""), "moai -C /repo note t-1 \"$(cat 리뷰)\"");
        // 앞의 환경변수 대입은 남긴다 — 이름은 감싸지 않는다(감싸면 셸이 명령으로 읽는다).
        assert_eq!(line("MOAI_ACTOR='이름 (a@b)' moai add '딴 일'"), "MOAI_ACTOR='이름 (a@b)' moai -C /repo add '딴 일'");
        // 쓰는 토막은 **모두** 낸다 — 첫 것만 내던 판은 닫는 `mv` 를 조용히 지웠다.
        assert_eq!(
            line("moai note t-1 'a' && moai mv t-1 done"),
            "moai -C /repo note t-1 a && moai -C /repo mv t-1 done"
        );
        // `--` 뒤의 `--help` 는 글이지 도움말이 아니다. `-m` 의 값도 마찬가지다.
        assert_eq!(line("moai note t-1 -- --help"), "moai -C /repo note t-1 -- --help");
        assert_eq!(line("moai mv t-1 done -m -h"), "moai -C /repo mv t-1 done -m -h");

        // 읽기·탐색기·도움말과 `moai` 가 아닌 줄은 지난다.
        for cmd in [
            "moai status",
            "moai show t-1 --tree",
            "moai tui",
            "moai read t-1",
            "moai mv t-1 done --help",
            "moai init argos",
            "cargo test && git commit -m '무엇을'",
        ] {
            assert_eq!(guard_tracker(cmd, &at), Decision::Pass, "막혔다 — {cmd}");
        }
        // **겨눌 루트가 없으면 안 막는다** — 맨 저장소에 딸린 워크트리와 서브모듈이 그 자리다.
        // 막으면 거절문이 낼 수 있는 줄은 막힌 줄뿐이라, 나갈 길이 도구 밖에만 남는다.
        assert_eq!(guard_tracker("moai note t-1 'x'", &|_| None), Decision::Pass);
    }
