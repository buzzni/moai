...
///
/// **미루거나 도로 집은 때(`planned_at`)는 어느 쪽에서도 안 센다**(moai-cxk8). 미룬 막음도
/// 막으므로(moai-2sea) 미뤄 둔 동안에도 막힘은 이어졌고 `blocked_by_deferred` 로 드러나
/// 있었다. 세면 막힌 줄이나 막는 줄을 `defer`→`--undo` 하는 것만으로 열흘 막힘이 0일로
/// 돌아간다 — `planned_at` 을 `status_since` 와 따로 둔 까닭(미루기가 방치 경고를 지우는
/// 손잡이)이 경고 쪽으로 돌아온다. 그래서 미뤘던 묶음을 도로 집으면 그것에 막힌 줄은
/// 곧장 "N일째" 로 선다 — 그 막힘은 실제로 이어졌던 것이다.
///
...
mod tests {
    ...
    fn blocked_stale_counts_from_when_the_block_came_back() {
        ...
        // **미루기·도로 집기는 막힘 시계를 새로 세우지 않는다**(moai-cxk8). 미룬 막음도 막으므로
        // (moai-2sea) 미뤄 둔 동안에도 막힘은 이어졌고 `blocked_by_deferred` 로 드러나 있었다.
        // 새로 세우면 `defer`→`--undo` 두 번이 열흘 막힘 경고를 지우는 손잡이가 된다.
        // 접어 둔 멤버를 오늘 도로 집었다.
