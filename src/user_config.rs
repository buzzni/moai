...
pub struct Look {
    ...
    ///
    /// **그 "모든" 은 2026-09-20 의 아홉 열로 얼렸다**(moai-4gy5, 사용자 결정 2026-09-18) — 얼린 목록은
    /// `crate::tui::view::Field::EMPTY_KNOWN` 이다. 이 바이너리가 아는 열로 읽으면 `[]` 의 뜻이
    /// 바이너리를 따라 움직여, 뒤에 더한 기본-켠 열이 그 설정에서 영영 안 뜨거나 파일을 먼저 만진
    /// 바이너리에 따라 갈린다. 얼린 목록 밖의 열은 기본값으로 서고, 그 설정을 다시 적어도 뜻이 안 바뀐다.
