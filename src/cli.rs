...
pub enum Cmd {
  ...
  --worktree 는 다른 git 워크트리의 이슈도 겹쳐 본다. 같은 id 는 칸을 늦게 옮긴
  줄이 서고(같으면 updated_at 이 늦은 줄), 지금 브랜치가 아닌 줄은 제목 앞에
  ⎇ <브랜치> 가 붙는다.
  ...
  것에 ⎇ <브랜치> 와 함께 선다. 같은 id 는 칸을 늦게 옮긴 줄로 읽으므로, 옆에서
  집은 뒤 여기서 제목·우선순위만 고쳐도 옆에서 집은 것이 풀리지 않는다.")]
    Ready(WorktreeArg),
...
제목이 `--` 로 시작해도 된다. 아는 플래그가 아니면 제목으로 읽는다.")]
  ...
  **칸도 종류도 안 바꾼다.** 어느 칸에 있었는지는 도로 집을 때 그대로
  ...
  a 는 디렉터리를 골라 프로젝트로 등록하는 창을 연다 — 띄운 자리에서 한 층씩
  드나들고(Enter·Backspace), `.moai` 가 있는 것과 이미 등록한 것에 표시가 붙는다.
  모노레포 하위도 고른 그대로 따로 선다. `.moai` 가 없어도 받는다. 프로젝트
  안에서도 열리므로 등록이 없어도 첫 프로젝트를 더할 수 있다. 층의 줄에서 d 는
  한 번 물은 뒤 목록에서만 뺀다 — 디렉터리와 `.moai` 는 그대로다. 쓰는 곳은
  `moai project add|rm` 과 같다.

  n 은 프로젝트 안 어디서든 생각 담기를 연다 — idea 로 담긴다(에픽 없이). 편집기
  ($VISUAL, $EDITOR, 없으면 PATH 의 vi 나 nano)가 있으면 git 커밋 메시지처럼 그것이
  뜬다: 첫 줄이 제목, 한 줄 띄우고 본문, 주석 줄은 안내라 지운다. 제목을 비우거나
  편집기를 오류로 끝내면 담지 않는다. 편집기가 없으면 안의 폼이 열린다 — 제목 한
  줄과 본문, Tab 이 둘 사이를 옮기고 Ctrl-S 나 F2 가 담는다. Esc 는 닫되 적던 것이
  있으면 한 번 묻는다. 이슈에 쓰는 것은 이것 하나다 — 고치는 것은 CLI 로 한다.
  `--json` 은 화면을 켜지 않고 그 디렉터리의 목록만 낸다(`.moai` 밖이면 층의 줄).")]
    Tui(TuiArgs),
