...
NOTE
  ...
  **칸도 종류도 안 바꾼다.** 어느 칸에 있었는지는 도로 집을 때 그대로
  ...
  층으로는 0 으로 간다 — Backspace 는 디렉터리만 올라간다. 층에서는 Enter 로 들어간다. 등록한 것이 없는데 밖에서 띄우면
  ...
  등록한 것에 표시가 붙는다. 창 안에서 a 가 커서의 디렉터리를 등록하고, `.` 이 숨은
  디렉터리를 보이거나 감추고, g p 가 경로를 직접 적는 칸을 연다(Enter 로 가고 Esc 로
  그만둔다). 창은 Esc 로 닫는다. 모노레포 하위도 고른 그대로 따로 선다. `.moai` 가
  없어도 받는다. 프로젝트 안에서도 열리므로 등록이 없어도 첫 프로젝트를 더할 수 있다.
  층의 줄에서 SPC p d 는 한 번 물은 뒤 목록에서만 뺀다 — y 가 예고 다른 키는
  그만둔다. 디렉터리와 `.moai` 는 그대로다. 쓰는 곳은 `moai project add|rm` 과 같다.

  SPC n 은 프로젝트 안 어디서든 생각 담기를 연다 — idea 로 담긴다(에픽 없이). 편집기
  ...
  줄과 본문, Tab 이 둘 사이를 옮기고(제목 칸의 Enter 는 본문으로 간다) Ctrl-S 가
  담는다. Esc 는 닫되 적던 것이 있으면 한 번 묻는다(y 로 버린다). 이슈에 쓰는 것은
  이것 하나다 — 고치는 것은 CLI 로 한다.
