...
NOTE
  ...
  **트래커에 안 쓴다.** 읽음은 사람마다 다른 값이라 이슈 줄에 적으면 읽기만 해도
  ...
  칸·가나다가 위고, 고른 것을 다시 누르면 거꾸로 선다. 기본(우선순위)이 아니면
  경로 줄이 그 차례를 댄다.
  ...
  `.moai` 밖에서 띄우면 층에서 시작하고, 안에서 띄우면 그 프로젝트 안에서
  시작한다. 층으로는 0 으로 간다 — Backspace 는 디렉터리만 올라간다. 층에서는
  Enter 로 들어간다. 등록한 것이 없는데 밖에서 띄우면 빈 층이 서서 SPC p a 로
  첫 프로젝트를 더하라고 댄다(`--json` 은 등록 없음으로 멈춘다).

  SPC p a 는 디렉터리를 골라 프로젝트로 등록하는 창을 연다 — 띄운 자리에서 한
  층씩 드나들고(Enter·Backspace, 이동은 목록과 같은 j·k·gg·G), `.moai` 가 있는
  것과 이미 등록한 것에 표시가 붙는다. 창 안에서 a 가 커서의 디렉터리를
  ...
  그만둔다. 디렉터리와 `.moai` 는 그대로다. 쓰는 곳은 `moai project add|rm` 과
  같다.
