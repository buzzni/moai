...
에픽 끝의 리뷰(`xhigh`·`max`)는 언제나 `opus` 다 — 멤버는 따로 안 보니 이 한 번이 그 에픽의
...
`<xhigh|max>` 는 채우지 않는다 — 7 에서 개발해 본 일꾼이 고르는 리뷰 등급의 자리다.
    ...
    5. **멤버마다 리뷰하지 않는다.** 멤버 하나가 끝나면 시험을 돌리고 커밋해 다음 멤버로
       ...
       같이 고친다.
       `low`·`medium`·`high` 은 머리의 모델을 고른 잣대이고, 7 의 등급을 고를 때도 이것으로 멤버를 잰다.
    ...
    7. 병합 전에 에픽 전체를 `/code-review <xhigh|max> --fix` 로 본다 — 멤버를 따로 안 봤으니
       ...
       하나라도 있으면** max. 망설여지면 max 다. 이 리뷰는 머리의 모델과 상관없이 `opus` 로 본다 — 창이
