...
- 그런 일이 있으면 **새 idea 보다 먼저** 놀고 있는 세션 하나에 이어 하기를 맡긴다. 3 의
  ...
  빠진다
      ...
      모델: <모델> (<난이도> — <까닭>) — 난이도로 고른 제안이다. 모델은 `/model` 로 바꾸는데 그것은
         ...
         어려우면 같은 길로 한 단계 올린다(haiku → sonnet → opus). 에픽 끝 리뷰의 등급(7)도 이 잣대로
         멤버를 재어 고른다
...
`<xhigh|max>` 는 채우지 않는다 — 7 에서 개발해 본 일꾼이 고르는 리뷰 등급의 자리다.
    ...
    7. 병합 전에 에픽 전체를 `/code-review <xhigh|max> --fix` 로 본다 — 멤버를 따로 안 봤으니
       ...
       **쓰기 경로·동시성·저장 형식·훅을 건드린 멤버가 하나라도 있으면** max. 망설여지면 max 다.
       이 리뷰는 머리의 모델과 상관없이 `opus` 로 본다 — 창이 `opus` 가 아니면 부르기 전에
       창을 보는 사람에게 `/model opus` 를 청한다(리뷰 에이전트는 창의 모델을 물려받는다).
