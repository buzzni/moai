...
**마일스톤이 돌면 그 안이 먼저다.** `moai ready` 의 머리가 무엇이 도는지와 밖에서
몇 건을 안 냈는지를 대고, `moai status` 의 알림도 같은 것을 비춘다. 그러면 이번 바퀴에
보낼 것은 그 마일스톤에 딸린 일이다 — 밖의 idea 는 `p0` 으로 세울 것이 아니면 다음
바퀴로 미룬다. **도구는 이것을 막지 않는다**(집기는 그대로 지나간다), 그래서 고르는
자리가 여기다. 도는 마일스톤이 둘이면 둘 다 안이다.
...
`<회사>`·`<수>` 도 채우지 않는다 — 9-1 에서 일꾼이 제 창에서 읽어 채우는 회사와 토큰이다.
    ...
    7-2. **마일스톤 안에서 돌린 에픽이면, 넘긴 것을 배포 기준으로 한 번 더 가른다.**
       리뷰는 배포 기준과 무관하게 내므로, 전부 안에서 고치면 배포가 리뷰 건수만큼 밀리고
       전부 밖으로 보내면 버그를 실은 채 배포한다. 가르는 것은 이 창이다 — 7 에서 무엇을
       반영하고 무엇을 넘길지 이미 정했으니 그 연장이다. **버그 수준이면 안에 둔다**: 그
       에픽의 멤버(`-e <에픽>`)나 같은 마일스톤의 에픽 밑에 세운다. **이번 배포에 없어도
       되면 밖으로 보낸다** — 안 하는 것이 아니라 이번 배포에 안 싣는 것이다
         moai -C <루트> edit <그 줄> -e none --milestone none
       **`-e none` 을 함께 준다.** 마일스톤은 에픽에서 물려받으므로, 에픽 멤버에 `--milestone
       none` 만 치면 "에픽에서 오는 자리라 안 끊긴다" 는 한 줄과 함께 아무것도 안 바뀐다 —
       7-1 이 되찾은 줄은 그 에픽의 멤버로 섰으니 여기서 에픽에서도 뺀다. 그 줄이 에픽을
       열어 두지 않게 되는 것은 뜻한 바다: 이번 배포에 안 싣기로 한 줄이다
       **버그 수준은 있는 낱말로 잰다** — `#bug` 태그가 붙는가, `Regression-of:` 줄을 적을
       수 있는가(이미 병합된 것이 깨졌는가). 그 둘이면 안, 나머지는 밖이다. 새 축을
       만들지 않는 까닭은 그것이 칸·종류·미룸 말고 넷째 어휘가 되어서다
